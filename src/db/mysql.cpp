...
int MySQL::execute(const char* format, va_list ap) {
    m_cmd = tao::StringUtil::Formatv(format, ap);
...
static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql) {
    if(mysql == nullptr) {
        TAO_LOG_ERROR(g_logger) << "mysql_query mysql is null";
        return nullptr;
    }

    if(sql == nullptr) {
        TAO_LOG_ERROR(g_logger) << "mysql_query sql is null";
        return nullptr;
    }

    if(::mysql_query(mysql, sql)) {
        TAO_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:"
            << mysql_error(mysql);
        return nullptr;
    }
    ...
    if(res == nullptr) {
        TAO_LOG_ERROR(g_logger) << "mysql_store_result() error:"
            << mysql_error(mysql);
    }
    return res;
}
...
MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string& stmt) {
    ...
    if(mysql_stmt_prepare(st, stmt.c_str(), stmt.size())) {
        TAO_LOG_ERROR(g_logger) << "stmt=" << stmt
...
int MySQLStmt::bindTime(int idx, const time_t& value) {
    return bindString(idx, tao::Time2Str(value));
}
...
int64_t MySQLRes::getInt64(int idx) {
    return tao::TypeUtil::Atoi(m_cur[idx]);
}
...
double MySQLRes::getDouble(int idx) {
    return tao::TypeUtil::Atof(m_cur[idx]);
}
...
int MySQLTransaction::execute(const char* format, va_list ap) {
    if(m_isFinished) {
        TAO_LOG_ERROR(g_logger) << "transaction is finished, format=" << format;
        return -1;
    }
...
int MySQLTransaction::execute(const std::string& sql) {
    if(m_isFinished) {
        TAO_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
        return -1;
    }
    ...
    if(it != m_conns.end()) {
        if(!it->second.empty()) {
            ...
            if(rt->ping()) {
                ...
                TAO_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                return nullptr;
            }
        }
    }
...
ISQLData::ptr MySQLManager::query(const std::string& name, const char* format, va_list ap) {
    auto conn = get(name);
    if(!conn) {
        TAO_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
            << ") fail, format=" << format;
        return nullptr;
    }
...
MySQLTransaction::ptr MySQLManager::openTransaction(const std::string& name, bool auto_commit) {
    auto conn = get(name);
    if(!conn) {
        TAO_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name
            << ") fail";
        return nullptr;
    }
