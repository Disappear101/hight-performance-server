...
// Socket::ptr Socket::CreateTCP(tao::Address::ptr address) {
...
//     if(rt) {
//         TAO_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
//         TAO_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
//             << " level=" << level << " option=" << option
//             << " errno=" << errno << " errstr=" << strerror(errno);
//         return false;
//     }
...
//     if(newsock == -1) {
//         TAO_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
...
//     if (!isValid()) {
//         newSock();
//         if(TAO_UNLIKELY(!isValid())) {
//             return false;
//         }
//     }
//     if (TAO_UNLIKELY(addr->getFamily() != m_family)) {
//         TAO_LOG_ERROR(g_logger) << "bind sock.family("
...
//         TAO_LOG_ERROR(g_logger) << "bind error errrno=" << errno
...
//             TAO_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
//     if (!isValid()) {
//         TAO_LOG_ERROR(g_logger) << "listen error sock=-1";
//         return false;
//     }
//     if (::listen(m_sock, backlog)) {
//         TAO_LOG_ERROR(g_logger) << "listen error errno=" << errno
//             << " errstr=" << strerror(errno);
//         return false;
//     }
//     return true;
...
//         TAO_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
//             << " errno=" << errno << " errstr=" << strerror(errno);
//         return Address::ptr(new UnknownAddress(m_family));
//     }
//     if(m_family == AF_UNIX) {
...
//     return IOManager::GetThis()->cancelEvent(m_sock, tao::IOManager::READ);
// }

// bool Socket::cancelWrite() {
//     return IOManager::GetThis()->cancelEvent(m_sock, tao::IOManager::WRITE);
// }
...
//     if (TAO_LIKELY(m_sock != -1)) {
//         initSock();
//     } else {
//         TAO_LOG_ERROR(g_logger) << "socket(" << m_family
...
bool Socket::bind(const Address::ptr addr) {
    ...
    //         return false;
    //     } else {
    //         tao::FSUtil::Unlink(uaddr->getPath(), true);
    //     }
    // }
...
//     if(!m_remoteAddress) {
//         TAO_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
//         return false;
//     }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(TAO_UNLIKELY(addr->getFamily() != m_family)) {
        TAO_LOG_ERROR(g_logger) << "connect sock.family("
            << m_family << ") addr.family(" << addr->getFamily()
            << ") not equal, addr=" << addr->toString();
        return false;
    }
...
Address::ptr Socket::getRemoteAddress() {
    ...
    socklen_t addrlen = result->getAddrLen();
    if(getpeername(m_sock, result->getAddr(), &addrlen)) {
        //TAO_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
        //    << " errno=" << errno << " errstr=" << strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
    if(m_family == AF_UNIX) {
