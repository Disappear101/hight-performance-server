...
int ZlibStream::write(ByteArray::ptr ba, size_t length) {
    ...
    ba->getReadableBuffers(buffers, length);
    if(m_encode) {
...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    TAO_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    TAO_ASSERT((window_bits >= 8 && window_bits <= 15));
    TAO_ASSERT((memlevel >= 1 && memlevel <= 9));
...
tao::ByteArray::ptr ZlibStream::getByteArray() {
